...
import AdminPricingControl from "./AdminPricingControl.jsx";
import {
...
function AdminBoardingTimeEditor({ boardingTime = "10:00", onUpdateBoardingTime }) {
  const preview = String(boardingTime || "10:00").trim();

  return (
    <section className="rounded-[2rem] border border-stone-200 bg-white p-6">
      <div>
        <p className="text-xs font-black tracking-[0.2em] text-orange-600">BOARDING TIME</p>
        <h3 className="mt-1 text-2xl font-black text-stone-950">탑승 기본 시간</h3>
        <p className="mt-2 text-sm font-bold leading-6 text-stone-600">
          예약확정 문자에 표시되는 탑승 시간입니다. 변경이 필요한 경우에만 수정하세요.
        </p>
      </div>
      ...
      <div className="mt-5 flex flex-col gap-4 md:flex-row md:items-center">
        <div className="flex flex-col gap-2">
          <label className="text-xs font-black text-stone-600">
            탑승 시간 <span className="text-orange-500">(예약확정 문자에 자동 반영)</span>
          </label>
          <input
            type="text"
            value={boardingTime || "10:00"}
            onChange={(e) => onUpdateBoardingTime?.(e.target.value)}
            placeholder="예) 10:00"
            maxLength={8}
            className="w-36 rounded-2xl border border-stone-200 bg-white px-4 py-3 text-center text-lg font-black text-stone-950 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
          />
        </div>

        <div className="rounded-2xl border border-orange-100 bg-orange-50 px-5 py-4">
          <p className="text-xs font-black text-stone-500 mb-1">예약확정 문자 미리보기</p>
          <p className="text-sm font-bold text-stone-700">
            탑승: 대전역 동광장{" "}
            <span className="font-black text-orange-700">{preview}</span>
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-[1.5rem] border border-orange-100 bg-orange-50 p-4">
        ...
        <p className="mt-1 text-xs font-bold text-stone-500">
          저장된 시간은 이후 발송되는 모든 예약확정 문자에 반영됩니다. 기본값은 10:00입니다.
        </p>
      </div>
    </section>
  );
...
function AdminDailyChecklist({ reservations = [], scheduleDetails = {}, scheduleStatus = {} }) {
  ...
  const checklist = [
    { label: "예약 목록 새로고침", done: reservations.length > 0, help: "운영 시작 전 최근 예약을 확인하세요." },
    { label: "모집중 날짜 확인", done: hasOpenSchedule, help: "고객에게 열려 있는 날짜가 있는지 확인하세요." },
    { label: "여행 일정 등록 확인", done: hasScheduleDetail, help: "고객 구매 전환을 위해 일정 내용을 등록하세요." },
    { label: "결제대기 예약 확인", done: !hasWaitingReservation, help: "결제대기 예약은 입금/결제 확인 후 상태를 변경하세요." }
  ];
    ...
    <section className="rounded-[2rem] border border-orange-200 bg-orange-50 p-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm font-bold leading-6 text-stone-600">예약 운영 전 꼭 확인하면 좋은 항목입니다.</p>
        </div>
...
export default function AdminDashboard({
  ...
  const summaryCards = useMemo(() => {
    const summary = createAdminDashboardSummary({ reservations: reservationsWithNotes, dateSettings, notificationQueue: [] });
    return getAdminDashboardSummaryCards(summary);
  }, [dateSettings, reservationsWithNotes]);

  const healthReport = useMemo(
    () => createAppHealthReport({ reservations: reservationsWithNotes, capacityOverrides, priceOverrides, scheduleStatus }),
  ...
  return (
    <section className="mt-10 rounded-[2.5rem] border border-stone-200 bg-stone-950 p-5 shadow-xl shadow-orange-100 md:p-8">
      <div className="mb-8 flex flex-col gap-4 text-white md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm font-black tracking-[0.25em] text-orange-300">ADMIN CENTER</p>
          <h2 className="mt-2 text-4xl font-black">관리자 운영 센터</h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            type="button"
            onClick={() => typeof onRefreshReservations === "function" && onRefreshReservations()}
      ...
      {isQuickReservationView ? (
        <div className="mb-6 rounded-[1.5rem] border border-orange-300/40 bg-orange-400/10 p-5 text-white">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-black tracking-[0.2em] text-orange-200">QUICK VIEW MODE</p>
              <h3 className="mt-1 text-xl font-black">최근 {quickReservationLimit}건 빠른보기 중입니다.</h3>
            ...
            <button
              type="button"
              onClick={() => typeof onClearQuickReservations === "function" && onClearQuickReservations()}
      ...
      <div className="grid gap-6">
        ...
        <AdminBoardingTimeEditor
          ...
          onUpdateBoardingTime={onUpdateBoardingTime}
        />
        ...
        <AdminPricingControl
          ...
          priceOverrides={priceOverrides}
          onUpdatePricingSettings={onUpdatePricingSettings}
        />
