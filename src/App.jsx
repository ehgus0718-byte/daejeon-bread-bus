...
function Admin({ config, setConfig, reservations, setReservations, authed, password, setPassword, login, logout, resetAll }) {
  if (!authed) return <main className="mx-auto flex min-h-[70vh] max-w-md items-center px-5"><Card className="w-full shadow-2xl shadow-orange-100"><div className="p-8"><div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-stone-900 text-2xl text-white">🔒</div><h1 className="text-3xl font-black">관리자 로그인</h1><p className="mt-2 text-sm text-stone-500">기본 비밀번호: admin1234</p><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && login()} placeholder="비밀번호" className="mt-6 h-12 w-full rounded-2xl border px-4" /><Button onClick={login} variant="dark" className="mt-4 h-12 w-full">로그인</Button></div></Card></main>;
