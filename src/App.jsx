...
const SUPABASE_URL =
  import.meta.env.VITE_SUPABASE_URL || "https://mnwimnwdilerkktizzqn.supabase.co";
const SUPABASE_ANON_KEY = "sb_publishable_Gj1aGVMKebdtAynKmtbxkA_CxH4NJvi";
...
const defaultConfig = {
  ...
  heroTitle: "빵 따라 떠나는\n대전 빵 여행",
  heroDescription: "성심당부터 대전 로컬 베이커리까지\n맛있는 하루를 즐기는 빵버스 투어",
...
function Home({ config, reservations, form, setForm, reserve, setNotice }) {
  ...
  return <main>
    <section className="mx-auto max-w-7xl px-5 py-12">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.08fr_.92fr] lg:items-start">
        <div><Calendar config={config} reservations={reservations} value={form.date} onSelect={(date) => updateForm({ date, people: Math.min(Number(form.people || 1), remaining(config, reservations, date) || 1) })} />
...
</select></td><td className="p-3"><Button variant="danger" onClick={() => setReservations(reservations.filter((x) => x.id !== r.id))}>삭제</Button></td></tr>)}</tbody></table></div></Card>; }
...
return;
  ...
  return <div className="min-h-screen bg-[#fff8ef] text-stone-900"><header className="sticky top-0 z-40 border-b border-orange-100 bg-white/90 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4"><button onClick={() => setMode("home")} className="flex items-center gap-3 text-left"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-orange-500 text-2xl text-white">🚌</div><div><p className="text-xl font-black">{config.title}</p><p className="text-xs font-bold text-stone-500">Daejeon Bakery Bus Tour</p></div></button><nav className="flex gap-2"><Button variant="ghost" onClick={() => setMode("home")}>예약하기</Button></nav></div></header>{notice && <div className="mx-auto mt-5 max-w-7xl px-5"><div className="rounded-2xl border border-orange-200 bg-white px-5 py-3 text-sm font-bold text-orange-700">{notice}</div></div>}{mode === "home" ? <><Home config={config} reservations={reservations} form={form} setForm={setForm} reserve={reserve} setNotice={setNotice} /><Footer config={config} /></> : <Admin config={config} setConfig={setConfig} reservations={reservations} setReservations={setReservations} authed={authed} password={password} setPassword={setPassword} login={login} logout={() => setAuthed(false)} resetAll={resetAll} />}</div>;
}
